...
// Default theme on first launch: signal.
...
export const useThemeStore = create<ThemeState>((set) => ({
  activeKey: DEFAULT_THEME_KEY,
  setTheme: (key: string) => set({ activeKey: key }),
}));

/**
 * Hook that returns the currently active Theme object. Every screen reads
 * colors through this so that calling `setTheme(key)` re-renders the whole app
 * with new colors instantly — nothing is hardcoded.
 */
export function useTheme(): Theme {
  const activeKey = useThemeStore((s) => s.activeKey);
  return THEMES.find((t) => t.key === activeKey) ?? THEMES[0];
}
